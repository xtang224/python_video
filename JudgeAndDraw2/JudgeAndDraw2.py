...
'''
usage: python JudgeAndDraw2.py yourTest.jpg 1/-1
'''
...
def gray(img):
   #first we want to save it as tempraworigin
   v = img.shape
   rows = v[0]
   cols = v[1]
   ...
   for i in range(rows):
      for j in range(cols):
         ...
         img2[i, j] = newAve
         result += str(img2[i,j]) + ","
   f = open("tempraworigin", "w")
   f.write(result[:-1])
   f.close()
   
   #then we want to resize it for judging
   tmpImg = resize(img)
...
if __name__ == '__main__':
    if len(sys.argv)<3:
        print ("usage: python JudgeAndDraw2.py yourPic.jpg label")
    ...
    for n in range(rows):
       for m in range(cols):
          img[n,m] = int(results[count])
          count += 1
    
    print ("rows = %d and cols = %d" % (rows, cols))
    rects = []
    stop = 0	
    for n in range(rows):
       if stop==1:
       ...
       firstLeftCol = 0
       firstRightCol = 0         	 
       #for m in range(int(0.523*40),int(0.523*40)+int(8.89*40)):	       
       for m in range(cols-1):	       
         #result += str(img2[n,m]) + ","
         if img[n,m]!=0 and img[n,m+1]!=0 and img[n,m]/2>img[n,m+1]:
            #print ("left border detected at row %d and column %d " % (n,m))
            if firstLeft == 0:
               firstLeftCol = m
               firstLeft = 1
         elif img[n,m]!=0 and img[n,m+1]!=0 and img[n,m]*2<img[n,m+1]:
            #print ("right border detected at row %d and column %d " % (n,m))
            if firstRight == 0:
               firstRightCol = m
               firstRight = 1
         if firstLeftCol!=0 and firstRightCol!=0 and firstRightCol-firstLeftCol>cols/32:
            if stop==0:
               print ("at row %d, the width between left and right is wide enough" % n)
               stop = 1
               rects.append(firstLeftCol-cols/32)
               rects.append(n-rows/32)
               rects.append(firstLeftCol-cols/32 + 8.89 * cols/32)
               rects.append(n-rows/32 + 21.33 * rows/32)
               draw_rects(img, rects, (0,255,0))
    cv2.imshow('capture %d' % (1), img)
    cv2.waitKey()
